  ...
  try {
    ...
    for (const user of users) {
      ...
      for (let i = 0; i < numSessions; i++) {
        ...
        const logoutTime = Math.random() > 0.3 ? new Date(loginTime.getTime() + Math.random() * 8 * 60 * 60 * 1000) : null; // 30% chance of active session
    ...
    if (adminUsers.length > 0 && regularUsers.length > 0) {
      ...
      for (const admin of adminUsers) {
        ...
        for (let i = 0; i < numActions; i++) {
          ...
          const description = actionDescriptions[actionTypes.indexOf(actionType)].replace('{email}', targetUser.email);
          ...
          try {
            ...
            console.error(`❌ Error creating admin action for admin ${admin.email}:`, error.message);
          }
        }
