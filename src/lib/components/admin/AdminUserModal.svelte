<script lang="ts">
	...
	export let setModalActiveTab: (tab: 'sessions' | 'activities' | 'stats') => void;
			...
			<div class="px-6 py-4 overflow-hidden flex flex-col h-full">
				...
				<div class="border-b border-gray-200 mb-6 flex-shrink-0">
					<nav class="-mb-px flex space-x-8">
						<button on:click={() => setModalActiveTab('sessions')} class="py-2 px-1 border-b-2 font-medium text-sm {modalActiveTab === 'sessions' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}">User Sessions ({userSessions.length})</button>
						<button on:click={() => setModalActiveTab('activities')} class="py-2 px-1 border-b-2 font-medium text-sm {modalActiveTab === 'activities' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}">User Activities ({userActivities.length})</button>
						<button on:click={() => setModalActiveTab('stats')} class="py-2 px-1 border-b-2 font-medium text-sm {modalActiveTab === 'stats' ? 'border-indigo-500 text-indigo-600' : 'border-transparent text-gray-500 hover:text-gray-700 hover:border-gray-300'}">Chat Stats</button>
					</nav>
				</div>
