	...
	function generateChartData() {
		...
		const registrationCounts = last6Months.map(month => {
			return users.filter(user => 
				user.createdAt.startsWith(month)
			).length;
		});
		...
		userRegistrationData = {
			...
			max: Math.max(...registrationCounts, 1)
		};
	}
...
<div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
	...
	<div class="bg-white rounded-lg shadow p-6">
		<h3 class="text-lg font-medium text-gray-900 mb-4">User Registration Trend (Last 6 Months)</h3>
		<div class="h-48 flex items-end justify-between px-4 pb-4">
			{#if userRegistrationData}
				{#each userRegistrationData.data as value, i}
					<div class="flex flex-col items-center">
						<div 
							class="bg-blue-500 rounded-t transition-all duration-300 hover:bg-blue-600"
							style="width: 25px; height: {Math.max((value / userRegistrationData.max) * 150, 4)}px;"
						></div>
						<span class="text-xs text-gray-600 mt-2">{userRegistrationData.labels[i]}</span>
