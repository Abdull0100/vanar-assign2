				...
				<div class="rounded-lg border border-destructive/20 bg-destructive/10 p-3">
					<div class="flex items-start gap-2">
						...
						<div class="text-sm text-destructive">
							<strong>PERMANENT DELETION:</strong> This entire conversation will be permanently
							erased from:
							...
							<p class="mt-2 font-medium">
								This action cannot be undone and provides guaranteed permanent erasure from all
								systems.
							</p>
						</div>
					</div>
...
{#if showDeleteAllModal && deleteAllTarget}
	<Dialog.Root
		...
		onOpenChange={(val) => {
			if (!val) onCloseDeleteAll();
		}}
	>
		<Dialog.Content class="w-[90vw] max-w-md rounded-2xl bg-card text-card-foreground shadow-xl">
			<div class="flex items-center justify-between border-b border-border px-6 py-4">
				<Dialog.Title class="text-lg font-medium text-destructive"
					>Clear All Chat History</Dialog.Title
				>
			...
			<div class="space-y-4 px-6 py-4">
				...
				<div class="rounded-lg border border-destructive/20 bg-destructive/10 p-3">
					<div class="flex items-start gap-2">
						<AlertTriangle class="mt-0.5 h-4 w-4 flex-shrink-0 text-destructive" />
						<div class="text-sm text-destructive">
							<strong>PERMANENT DELETION:</strong> ALL conversations and messages will be
							permanently erased from:
							<ul class="mt-1 ml-4 list-disc">
								<li>Your local chat history</li>
								<li>The database</li>
								<li>All backup systems</li>
