...
export const GET: RequestHandler = async () => {
	try {
		...
		const todayConversations = await db.select({ count: sql<number>`count(*)` })
			.from(conversations)
			.where(gte(conversations.createdAt, today));
		
		// Get today's logins (from user activities)
		const todayLogins = await db.select({ count: sql<number>`count(*)` })
			.from(userActivities)
			.where(
		...
		for (let i = 6; i >= 0; i--) {
			...
			const dayConversations = await db.select({ count: sql<number>`count(*)` })
				.from(conversations)
				.where(
		...
		activityBreakdown[0].count = allUserStats.reduce((sum, stat) => sum + (stat.totalChatMessages || 0), 0);
		activityBreakdown[1].count = allUserStats.reduce((sum, stat) => sum + (stat.profileUpdateCount || 0), 0);
		activityBreakdown[2].count = allUserStats.reduce((sum, stat) => sum + (stat.passwordChangeCount || 0), 0);
