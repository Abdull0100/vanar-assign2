...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		const _history: HistoryTurn[] | undefined = Array.isArray(body.history)
			? body.history
			: undefined;
		...
		// Determine the appropriate previousId based on conversation state
		let previousId = null;
		...
		// Build conversation context from recent messages for AI prompt
		...
		const buildThreadedTranscript = (messages: any[]) => {
			...
			messages.forEach((msg) => {
				if (msg.userId === session.user!.id && msg.conversationId === targetConversationId) {
					messageMap.set(msg.id, msg);
					validMessageIds.add(msg.id);
				} else {
					console.warn(
						`Skipping unauthorized message: ${msg.id} (userId: ${msg.userId}, conversationId: ${msg.conversationId})`
					);
				}
			});
			...
			while (
				currentMessage &&
				!processedIds.has(currentMessage.id) &&
				validMessageIds.has(currentMessage.id)
			) {
				...
				currentMessage =
					currentMessage.previousId && validMessageIds.has(currentMessage.previousId)
		...
		const hasDocuments = await ragService.hasProcessedDocuments(
			session.user.id,
		...
		if (hasDocuments) {
			...
			retrievedChunks = await ragService.retrieveRelevantChunks(
				message,
				session.user.id,
				5,
				0.6,
...
${hasDocuments ? 'Please provide a helpful response based on the available context and document information when relevant. If you use information from the documents, be specific about which document the information comes from.' : 'Please provide a helpful response. Keep it conversational and relevant to the context. Do not mention anything about documents not being found or unavailable.'}`;
				...
				try {
					...
					if (session.user && targetConversationId) {
						...
						if (assistantTreeNode.parentId) {
							...
							const updatedChildrenIds = treeParent
								? treeParent.childrenIds
								: assistantTreeNode.childrenIds;
						...
						if (retrievedChunks.length > 0) {
							try {
								...
								console.log(
									`Created ${retrievedChunks.length} citations for message ${assistantTreeNode.id}`
								);
							} catch (citationError) {
		...
		if (error.status === 429) {
			// Rate limit exceeded
			errorMessage =
				"I'm currently experiencing high demand and can't process your request right now. Please try again in a few minutes. (Rate limit exceeded)";
		} else if (error.status === 403) {
			// API key issues or quota exceeded
			errorMessage =
				'The AI service is temporarily unavailable due to configuration issues. Please try again later.';
...
export const GET: RequestHandler = async ({ locals, url }) => {
	try {
		...
		if (conversationId) {
			...
			const virtualRoots = treeManager
				.getAllMessages()
				.filter((msg) => msg.id.startsWith('virtual-root-'));
			...
			for (const message of treeMessages) {
				if (
					message.childrenIds &&
					message.childrenIds.length > 1 &&
					!branchNavigationData.find((nav) => nav.messageId === message.id)
				) {
					// Find which child is currently in the active path
					let currentChildIndex = 0;
					for (let i = 0; i < message.childrenIds.length; i++) {
			...
			case 'fork_message':
				try {
					...
					if (!originalMessage) {
						...
						console.log(
							'Available messages in tree manager:',
							Array.from(treeManager.getAllMessages()).map((m) => ({
								id: m.id,
						...
						console.log(
							...
							allMessages.map((m) => ({ id: m.id, content: m.content.substring(0, 50) }))
						);
					...
					if (forkedMessage.parentId) {
						try {
							...
							if (parent) {
								...
								const updatedChildrenIds = treeParent
									? treeParent.childrenIds
									: [...(parent.childrenIds || []), forkedMessage.id];
...
Please provide a helpful response. Keep it conversational and relevant to the context.`;
							...
							for (const message of treeMessages) {
								if (
									message.childrenIds &&
									message.childrenIds.length > 1 &&
									!finalBranchNavigation.find((nav) => nav.messageId === message.id)
								) {
									let currentChildIndex = 0;
									for (let i = 0; i < message.childrenIds.length; i++) {
										const childId = message.childrenIds[i];
			...
			case 'switch_branch':
				...
				if (messageId.startsWith('virtual-root-')) {
					...
					if (branchIndex < 0 || branchIndex >= rootMessages.length) {
						throw new ValidationError(
							`Invalid branch index ${branchIndex} for ${rootMessages.length} root messages`
						);
					}

					// Sort root messages by creation time to ensure consistent ordering
					const sortedRoots = rootMessages.sort(
						(a, b) => new Date(a.createdAt).getTime() - new Date(b.createdAt).getTime()
					);
					const targetRootMessage = sortedRoots[branchIndex];

					console.log(
						'Switching to root message:',
						targetRootMessage.id,
						'content:',
						targetRootMessage.content.substring(0, 30)
					);
				...
				for (const message of treeMessages) {
					if (
						message.childrenIds &&
						message.childrenIds.length > 1 &&
						!switchedBranchNavigationData.find((nav) => nav.messageId === message.id)
					) {
						// Find which child is currently in the active path
						let currentChildIndex = 0;
						for (let i = 0; i < message.childrenIds.length; i++) {
